...
import {Ciphertext, PendingRequest, EncryptionOracle, IEncryptionOracle, IEncryptionClient, RequestDoesNotExist, OracleResultFailed, NotRelayer, NotRelayerOrOwner, ReencryptedCipher} from "../src/EncryptionOracle.sol";
...
contract MockEncryptionClient is IEncryptionClient {
    ...
    function oracleResult(uint256 requestId, ReencryptedCipher memory _cipher)
        external
    {
        if (shouldRevert) {
            revert("I messed up");
...
contract EncryptionOracleTest is Test {
    ...
    function cipher2Reenc(Ciphertext memory c)
        private
        pure
        returns (ReencryptedCipher memory)
    {
